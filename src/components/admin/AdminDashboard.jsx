  ...
  return (
    <div className="p-8">
      <motion.div
        ...
        className="mb-8"
      >
        <h1 className="text-3xl font-bold mb-1" style={{ color: '#F5F5F7', letterSpacing: '-0.02em' }}>Дашборд</h1>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex gap-2">
            {['День', 'Неделя', 'Месяц'].map((period, i) => (
              <button
                key={period}
                className="text-xs px-3 py-1 rounded-full"
                style={{
                  background: i === 0 ? 'rgba(10,132,255,0.2)' : 'rgba(255,255,255,0.06)',
                  color: i === 0 ? '#0A84FF' : '#98989D',
                }}
              >
                {period}
              </button>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={revenueData}>
            ...
            />
            <Area type="monotone" dataKey="revenue" stroke="#0A84FF" strokeWidth={2.5} fill="url(#revGrad)" />
          </AreaChart>
        </ResponsiveContainer>
      </motion.div>
